...
import com.example.doan.Model.LessonProgress;
...
/**
 * Controller phục vụ Học viên xem danh sách, chi tiết khóa học, lớp học, khóa học cá nhân và khảo sát gợi ý AI.
 * File: Controller/StudentCourseController.java
 */
@Controller
    ...
    public String detail(@PathVariable("id") Long id, Model model) {
        ...
        if (auth != null && auth.isAuthenticated() && !auth.getName().equals("anonymousUser")) {
            ...
            if (currentUser != null) {
                ...
                isEnrolled = hasBought || "admin".equals(currentUser.getRole());
                hasReviewed = reviewService.Has_Reviewed(currentUser.getId(), id);
                canReview = hasBought && !hasReviewed && !"admin".equals(currentUser.getRole());
            }
        }
    ...
    public String learn(@PathVariable("id") Long id, Model model) {
        ...
        if (enrollment == null) {
            if ("admin".equals(currentUser.getRole())) {
                // Tự động tham gia khóa học cho Admin để Admin có quyền vào hỗ trợ học viên / trả lời câu hỏi
                enrollment = new Enrollment();
                enrollment.setUser(currentUser);
                enrollment.setCourse(courseService.Get_ById(id));
