...
public class Course_Controller {
    ...
    public String postDelete(@PathVariable("id") Long id, @RequestParam(value = "reason", required = false) String reason) {
    ...
    // 7. Duyệt khóa học (Chuyển pending_review -> active)
    @PostMapping("/approve/{id}")
    public String approveCourse(@PathVariable("id") Long id) {
        Course course = courseService.Get_ById(id);
        if (course != null && "pending_review".equals(course.getTrangThai())) {
    ...
    // 8. Từ chối khóa học (Chuyển pending_review -> draft)
